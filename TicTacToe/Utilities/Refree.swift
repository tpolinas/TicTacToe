...
public final class Referee {
    ...
    private func generateWinsByColumn(
        result: inout [[GameboardPosition]]
    ) {
        if fieldSize == 3 {
            for column in 0 ..< fieldSize {
                var array: [GameboardPosition] = []
                for row in 0 ..< fieldSize {
                    array.append(GameboardPosition(
                        column: column,
                        row: row))
                }
                result.append(array)
    ...
    private func generateWinsByRow(
        result: inout [[GameboardPosition]]
    ) {
        if fieldSize == 3 {
            for row in 0 ..< fieldSize {
    ...
    private func generateWinLeftDiagonal(
        result: inout [[GameboardPosition]]
    ) {
        ...
        if fieldSize == 3 {
            ...
            for i in 0 ..< fieldSize - 3 {
                array[0].append(GameboardPosition(
                    column: i,
                    row: i))
                array[1].append(GameboardPosition(
                    column: i,
                    row: i + 1))
                array[2].append(GameboardPosition(
                    column: i,
                    row: i + 2))
                array[3].append(GameboardPosition(
                    column: i,
                    row: i + 3))
            }
    ...
    private func generateWinRightDiagonal(
        result: inout [[GameboardPosition]]
    ) {
    ...
    private func doesPlayerHaveWinningCombination(
        _ player: Player
    ) -> Bool {
        for winningPositions in winningCombinations {
            if gameboard.contains(
                player: player,
                at: winningPositions) { return true }
        }
        return false
    }
}
